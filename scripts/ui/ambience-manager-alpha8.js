...
export function resolveManagerAction(action, selectedId = "") {
  const normalizedAction = String(action ?? "").trim();
  return {
    action: normalizedAction,
    id: normalizedAction === "new" ? "" : String(selectedId ?? "").trim()
  };
}

function bindManagerActions(dialog, api) {
  const root = dialog.element;
  if (!root) return;

  const selection = root.querySelector?.('select[name="ambienceId"]') ?? null;
  const actionButtons = [...(root.querySelectorAll?.("[data-af-manager-action]") ?? [])];

  // Snapshot the live selection onto every action button and keep that snapshot
  // current. This avoids relying on DialogV2 form submission or on querying a
  // form which may have been rebuilt by Foundry between pointerdown and click.
  const syncSelection = () => {
    const selectedId = String(selection?.value ?? "").trim();
    for (const actionButton of actionButtons) {
      const action = String(actionButton.dataset.afManagerAction ?? "");
      actionButton.dataset.afAmbienceId = action === "new" ? "" : selectedId;
    }
  };
  ...
  for (const actionButton of actionButtons) {
    actionButton.addEventListener("click", async (event) => {
      ...
      const result = resolveManagerAction(
        actionButton.dataset.afManagerAction,
        actionButton.dataset.afAmbienceId
      );

      // Alpha diagnostic: this line also proves that the newly-versioned
      // manager script is actually the one running rather than a cached ESM.
      console.info("ambience-forge | manager alpha.8 action", result);

      await dialog.close();
      await handleManagerResult(api, result);
    });
  }
}
...
export async function openAmbienceManager(api) {
  ...
  dialog.addEventListener("render", () => bindManagerActions(dialog, api));
