...
class EmitterCanvasOverlay {
  ...
  #createMarker(emitter) {
    ...
    marker._afSuppressClickUntil = 0;
  ...
  #updateMarker(marker, emitter) {
    ...
    main.on("pointertap", (event) => {
      event.stopPropagation?.();
      if (Date.now() < (marker._afSuppressClickUntil ?? 0)) return;
  ...
  async #dragEnd(event) {
    ...
    event.preventDefault?.();
    // PIXI may dispatch pointertap shortly after pointerup. A zero-delay flag reset
    // can therefore re-enable click-to-edit before that pointertap arrives. Keep a
    // short suppression window after a real drag so moving an emitter only moves it.
    drag.marker._afSuppressClickUntil = Date.now() + 300;
