...
function activeCard(api, ambience, state) {
  ...
  if (enabledTracks.length) {
    card.append(element("p", {
      className: "ambience-forge-quick-tracks",
      text: game.i18n.format("AMBIENCE_FORGE.Quick.Tracks", {
        tracks: enabledTracks.map((track) => track.name || game.i18n.localize("AMBIENCE_FORGE.Editor.UnnamedTrack")).join(", ")
      })
    }));
  }
  ...
  card.append(field(
    ...
    game.i18n.localize("AMBIENCE_FORGE.Quick.MasterVolumeHint")
  ));

  for (const track of ambience.tracks.filter((candidate) => candidate.enabled !== false && candidate.type === TRACK_TYPES.INTENSITY)) {
    const intensity = state.trackIntensities?.[ambience.id]?.[track.id] ?? track.intensity ?? 0;
    card.append(field(
      game.i18n.format("AMBIENCE_FORGE.Quick.Intensity", { name: track.name || game.i18n.localize("AMBIENCE_FORGE.Editor.UnnamedTrack") }),
      volumeSlider(`quickIntensity-${ambience.id}-${track.id}`, Math.round(intensity * 100), {
        "af-quick-intensity": track.id,
