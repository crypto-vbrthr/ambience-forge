...
function getManagerAppClass() {
  ...
  ManagerAppClass = class AmbienceForgeManagerApp extends ApplicationV2 {
    ...
    async _onRender(context, options) {
      ...
      if (this.mode === "sequence-track") this.#bindSequenceTrack();
    ...
    async #editorAction(action) {
      ...
      if (action === "edit-track") {
        ...
        if (![TRACK_TYPES.AUDIO, TRACK_TYPES.RANDOM, TRACK_TYPES.SEQUENCE].includes(track.type)) {
...
function sequenceTrackContent(track) {
  ...
  wrapper.append(field("AMBIENCE_FORGE.Sequence.Volume", input("volumePercent", Math.round((track.volume ?? 1) * 100), { type: "number", min: 0, max: 100, step: 1 })));
