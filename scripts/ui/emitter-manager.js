...
function editorContent(api, draft, isNew) {
  ...
  wrapper.append(field(
    game.i18n.localize("AMBIENCE_FORGE.Emitter.ObstructionMode"),
    select("obstructionMode", [
      { value: "ignore", label: game.i18n.localize("AMBIENCE_FORGE.Emitter.ObstructionIgnore") },
      { value: "attenuate", label: game.i18n.localize("AMBIENCE_FORGE.Emitter.ObstructionAttenuate") },
      { value: "block", label: game.i18n.localize("AMBIENCE_FORGE.Emitter.ObstructionBlock") }
    ], draft.obstructionMode ?? "ignore"),
    game.i18n.localize("AMBIENCE_FORGE.Emitter.ObstructionModeHint")
  ));
  const attenuationField = field(
    game.i18n.localize("AMBIENCE_FORGE.Emitter.ObstructionAttenuation"),
    volumeSlider("obstructionAttenuationPercent", Math.round((draft.obstructionAttenuation ?? 0.7) * 100)),
    game.i18n.localize("AMBIENCE_FORGE.Emitter.ObstructionAttenuationHint")
  );
  attenuationField.dataset.afObstructionAttenuation = "";
  wrapper.append(attenuationField);
...
function readEditor(root, draft = {}) {
  ...
  return {
    ...
    obstructionMode: String(value("obstructionMode") ?? draft.obstructionMode ?? "ignore"),
    obstructionAttenuation: Math.min(100, Math.max(0, Number(value("obstructionAttenuationPercent") ?? Math.round((draft.obstructionAttenuation ?? 0.7) * 100)) || 0)) / 100
  };
}
  ...
  EmitterManagerClass = class AmbienceForgeEmitterManager extends ApplicationV2 {
    ...
    showEditor(emitterId = null) {
      ...
      this.draft = existing ? structuredClone(existing) : {
        ...
        easing: true,
        enabled: true,
        obstructionMode: "ignore",
        obstructionAttenuation: 0.7,
        x: null,
        y: null
      };
      return this.render(true);
    ...
    #bind() {
      ...
      const obstructionSelect = this.root?.querySelector?.('[name="obstructionMode"]');
      const syncObstructionFields = () => {
        const attenuation = this.root?.querySelector?.('[data-af-obstruction-attenuation]');
        if (attenuation) attenuation.hidden = obstructionSelect?.value !== "attenuate";
      };
      obstructionSelect?.addEventListener("change", syncObstructionFields);
      syncObstructionFields();
