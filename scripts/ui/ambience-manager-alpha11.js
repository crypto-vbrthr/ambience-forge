  ...
  ManagerAppClass = class AmbienceForgeManagerApp extends ApplicationV2 {
    ...
    async #editorAction(action) {
      ...
      if (action === "add-audio") {
        await this.close();
        managerApp = null;
        return openAudioTrackEditor(this.api, saved.id, null);
      }
      if (action === "add-random") {
        await this.close();
        managerApp = null;
        return openRandomTrackEditor(this.api, saved.id, null);
      }
      if (action === "edit-track") {
        ...
        await this.close();
        managerApp = null;
        if (track.type === TRACK_TYPES.RANDOM) return openRandomTrackEditor(this.api, saved.id, track.id);
        return openAudioTrackEditor(this.api, saved.id, track.id);
      }
...
function bindAudioSourcePicker(dialog) {
...
export async function openAudioTrackEditor(api, ambienceId, trackId = null, draft = null) {
  ...
  const track = draft ?? existing ?? normalizeTrack({ type: TRACK_TYPES.AUDIO, name: game.i18n.localize("AMBIENCE_FORGE.Audio.DefaultName"), repeat: true });
  const DialogV2 = foundry.applications.api.DialogV2;
  const dialog = new DialogV2({
    window: { title: existing ? game.i18n.format("AMBIENCE_FORGE.Audio.TitleEdit", { name: existing.name }) : game.i18n.localize("AMBIENCE_FORGE.Audio.TitleNew") },
    position: { width: 660 },
    content: audioTrackContent(track),
    buttons: [
      {
        action: "preview",
        label: game.i18n.localize("AMBIENCE_FORGE.Common.Preview"),
        icon: "fa-solid fa-headphones",
        callback: (event, button) => ({ action: "preview", track: readAudioTrackForm(button.form, track) })
      },
      {
        action: "stop-preview",
        label: game.i18n.localize("AMBIENCE_FORGE.Audio.StopPreview"),
        icon: "fa-solid fa-stop",
        callback: (event, button) => ({ action: "stop-preview", track: readAudioTrackForm(button.form, track) })
      },
      {
        action: "save",
        ...
        default: true,
        callback: (event, button) => ({ action: "save", track: readAudioTrackForm(button.form, track) })
      },
      {
        action: "back",
        label: game.i18n.localize("AMBIENCE_FORGE.Common.Back"),
        icon: "fa-solid fa-arrow-left",
        callback: (event, button) => ({ action: "back", track: readAudioTrackForm(button.form, track) })
      }
    ],
    submit: (result) => void handleAudioResult(api, ambienceId, trackId, result)
  });
  dialog.addEventListener("render", () => bindAudioSourcePicker(dialog));
  return dialog.render(true);
}

async function handleAudioResult(api, ambienceId, trackId, result) {
  if (!result) return;
  ...
  if (result.action === "stop-preview") {
    await api.stopPreview();
    return openAudioTrackEditor(api, ambienceId, trackId, result.track);
  }
  if (result.action === "preview") {
    if (!result.track.source) {
      ...
      try {
        await api.previewAudio(result.track);
      } catch (error) {
  ...
  if (result.action === "save") {
    ...
    const ambience = api.getAmbience(ambienceId);
    if (!ambience) return openAmbienceManager(api);
    const tracks = ambience.tracks.filter((candidate) => candidate.id !== result.track.id);
...
function readRandomTrackForm(form, existing) {
  const list = form.elements.sourceList;
  const sources = list ? Array.from(list.options).map((option) => String(option.dataset.source ?? option.textContent ?? "").trim()).filter(Boolean) : cloneData(existing?.sources ?? []);
  return normalizeTrack({
    ...(existing ?? {}),
    ...
    fadeInMs: 0,
    fadeOutMs: 150,
    minDelayMs: milliseconds(form.elements.minDelaySeconds?.value ?? 30),
    maxDelayMs: milliseconds(form.elements.maxDelaySeconds?.value ?? 120),
    avoidImmediateRepeat: Boolean(form.elements.avoidImmediateRepeat?.checked),
    allowOverlap: Boolean(form.elements.allowOverlap?.checked),
    sources
  });
}
...
export async function openRandomTrackEditor(api, ambienceId, trackId = null, draft = null) {
  ...
  if (!ambience) return openAmbienceManager(api);
  const existing = trackId ? ambience.tracks.find((track) => track.id === trackId) : null;
  const track = draft ?? existing ?? normalizeTrack({ type: TRACK_TYPES.RANDOM, name: game.i18n.localize("AMBIENCE_FORGE.Random.DefaultName") });
  const DialogV2 = foundry.applications.api.DialogV2;
  const dialog = new DialogV2({
    window: { title: existing ? game.i18n.format("AMBIENCE_FORGE.Random.TitleEdit", { name: existing.name }) : game.i18n.localize("AMBIENCE_FORGE.Random.TitleNew") },
    position: { width: 700 },
    content: randomTrackContent(track),
    buttons: [
      {
        action: "add-source",
        label: game.i18n.localize("AMBIENCE_FORGE.Random.AddSound"),
        icon: "fa-solid fa-folder-plus",
        callback: (event, button) => ({ action: "add-source", track: readRandomTrackForm(button.form, track) })
      },
      {
        action: "remove-source",
        label: game.i18n.localize("AMBIENCE_FORGE.Random.RemoveSound"),
        icon: "fa-solid fa-minus",
        callback: (event, button) => ({
          action: "remove-source",
          track: readRandomTrackForm(button.form, track),
          sourceIndex: Number(button.form.elements.sourceList?.value ?? -1)
        })
      },
      {
        action: "preview",
        label: game.i18n.localize("AMBIENCE_FORGE.Common.Preview"),
        icon: "fa-solid fa-headphones",
        callback: (event, button) => ({ action: "preview", track: readRandomTrackForm(button.form, track) })
      },
      {
        action: "stop-preview",
        label: game.i18n.localize("AMBIENCE_FORGE.Random.StopPreview"),
        icon: "fa-solid fa-stop",
        callback: (event, button) => ({ action: "stop-preview", track: readRandomTrackForm(button.form, track) })
      },
      {
        action: "save",
        label: game.i18n.localize("AMBIENCE_FORGE.Common.Save"),
        icon: "fa-solid fa-floppy-disk",
        default: true,
        callback: (event, button) => ({ action: "save", track: readRandomTrackForm(button.form, track) })
      },
      {
        action: "back",
        label: game.i18n.localize("AMBIENCE_FORGE.Common.Back"),
        icon: "fa-solid fa-arrow-left",
        callback: (event, button) => ({ action: "back", track: readRandomTrackForm(button.form, track) })
      }
    ],
    submit: (result) => void handleRandomResult(api, ambienceId, trackId, result)
  });
  return dialog.render(true);
}

async function handleRandomResult(api, ambienceId, trackId, result) {
  if (!result) return;
  if (result.action === "back") {
    await api.stopPreview();
    return openAmbienceEditor(api, ambienceId);
  }
  if (result.action === "add-source") {
    await api.stopPreview();
    return openRandomAudioPicker(api, ambienceId, trackId, result.track);
  }
  if (result.action === "remove-source") {
    await api.stopPreview();
    const sources = [...(result.track.sources ?? [])];
    if (Number.isInteger(result.sourceIndex) && result.sourceIndex >= 0 && result.sourceIndex < sources.length) sources.splice(result.sourceIndex, 1);
    return openRandomTrackEditor(api, ambienceId, trackId, { ...result.track, sources });
  }
  if (result.action === "stop-preview") {
    await api.stopPreview();
    return openRandomTrackEditor(api, ambienceId, trackId, result.track);
  }
  if (result.action === "preview") {
    if (!result.track.sources?.length) {
      ui.notifications.warn(game.i18n.localize("AMBIENCE_FORGE.Notifications.SourcesRequired"));
    } else {
      try {
        await api.previewRandom(result.track);
      } catch (error) {
...
async function openRandomAudioPicker(api, ambienceId, trackId, draft) {
  const FilePicker = foundry.applications.apps.FilePicker;
  let selected = false;
  const picker = new FilePicker({
    type: "audio",
    ...
    callback: (path) => {
      selected = true;
      const sources = [...(draft.sources ?? []), path];
      void openRandomTrackEditor(api, ambienceId, trackId, { ...draft, sources });
    }
  });
  picker.addEventListener("close", () => {
    if (!selected) void openRandomTrackEditor(api, ambienceId, trackId, draft);
  }, { once: true });
